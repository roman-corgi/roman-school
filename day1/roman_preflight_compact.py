...
# Version 2.0, July 2024, John Krist: now roman_preflight_compact; removed default HLC DM patterns
#    option; now output at orientation matching CCD frame; changed DM tilt axis; DM2 is offset from 
#    DM1 by 0.1 actuator in X.
...
from roman_preflight_proper import trim, ffts, mft2
...
##########################################################################
def roman_preflight_compact( lambda_m, output_dim0, PASSVALUE={'dummy':0} ):
    ...
    data_dir = roman_preflight_proper.data_dir
    ...
    dm1_yc_act = 23.5              
    dm1_xtilt_deg = 9.65        # tilt around X axis (deg)
    dm1_ytilt_deg = 0.          # effective DM tilt in deg including 9.65 deg actual tilt and pupil ellipticity
    ...
    dm2_yc_act = 23.5              
    dm2_xtilt_deg = 9.65        # tilt around X axis (deg)
    dm2_ytilt_deg = 0.          # effective DM tilt in deg including 9.65 deg actual tilt and pupil ellipticity
    ...
    if cor_type.find('hlc') != -1:
        ...
        if use_fpm != 0:
            ...
            w = np.argmin( diff )
            if diff[w] > 0.0001:
                print("Error in roman_preflight_compact: requested wavelength not within 0.1 nm of nearest available FPM wavelength.")
        ...
        raise Exception( 'roman_preflight_compact: Unsuported cor_type: '+cor_type )
    ...
    if use_dm2 != 0: proper.prop_dm( wavefront, dm2_m, dm2_xc_act, dm2_yc_act, dm_sampling_m, XTILT=dm2_xtilt_deg, YTILT=dm2_ytilt_deg, ZTILT=dm2_ztilt_deg, FLIP_LR=1 )
    ...
    wavefront[:,:] = np.transpose( wavefront )
